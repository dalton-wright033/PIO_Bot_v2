...
This bot is intended for LOCAL USE ONLY, and precautions have been taken to ensure personal information is not shared outside of the local machine and the messaging interface used (i.e. Discord). If using this for your own child (or self), please be sure to keep your information secure. 
...
- Integration with Ollama LLM for more dynamic story telling 
...
- Please be sure to install the following dependencies into your python environment before attempting to launch PIO using the command ```pip install [dependency_name]```. Otherwise you may get errors:
    - discord.py
    - requests
    - dotenv
...
- After creating your bot, run ```python3 main_PIO.py``` in the terminal (linux/MacOS) or Command Line (Windows)
    - Please make sure you have Python 3 downloaded to ensure code execution
...
- We have tested and improved upon Pio's safety in conversations with children. We still recommend parental supervision for two reasons
...
- I have some custom prompts that tell Pio's purpose that serve as a note to my daughter. Pio will state its purpose when asked who it is. If you wish to remove this, alter the core_identity variable in the generate_response() function in the llm_brain.py file. Leaving or removing this prompt will not affect the PIO's function.
